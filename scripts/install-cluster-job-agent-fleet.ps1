...
$Common = @('-o','BatchMode=yes','-o','ConnectTimeout=8','-o','ServerAliveInterval=5')
if ($SkipHostKeyCheck) { $Common += @('-o','StrictHostKeyChecking=accept-new') }
...
foreach ($Node in $Nodes) {
    ...
    if ($ApiKey) {
        $Remote = "export CLUSTER_API_KEY='$ApiKey'; curl -fsSL '$Installer' | sh -s -- '$($Node.Name)'"
    } else {
        $Remote = "curl -fsSL '$Installer' | sh -s -- '$($Node.Name)' </dev/null"
    }
    $Output = & ssh @Common -p $Node.Port $Target $Remote 2>&1
    $Code = $LASTEXITCODE
    $Ok = $Code -eq 0
    $Results.Add([pscustomobject]@{
        ...
        IP = $Node.Address
        Port = $Node.Port
        Status = $(if ($Ok) { 'ONLINE' } else { 'FAILED' })
        Detail = (($Output | Select-Object -Last 3) -join ' | ')
    })
    if ($Ok) { Write-Host ('  ONLINE - ' + $Node.Name) -ForegroundColor Green }
    else { Write-Host ('  FAILED - ' + $Node.Name + ' (SSH exit ' + $Code + ')') -ForegroundColor Red }
}
...
if ($Failed.Count -gt 0) {
    ...
    foreach ($Item in $Failed) { Write-Host ('- ' + $Item.Node + ': ' + $Item.Detail) }
    Write-Host 'Re-run this same script after the failed node is awake/reachable.' -ForegroundColor Yellow
    exit 1
}
...
if (-not $ApiKey) {
    Write-Host 'No local API key supplied; each node will reuse its existing ~/.cluster-env key.' -ForegroundColor DarkGray
}
