...
MINING_START_PC_CMD='WORKER=$(hostname 2>/dev/null || echo PC-NODE); XMRIG_BIN=$(command -v xmrig 2>/dev/null); [ -z "$XMRIG_BIN" ] && [ -x /usr/local/bin/xmrig ] && XMRIG_BIN=/usr/local/bin/xmrig; if [ -z "$XMRIG_BIN" ]; then echo "Downloading xmrig..."; ARCH=$(uname -m); if [ "$ARCH" = "x86_64" ]; then TARBALL="xmrig-6.21.3-linux-x64.tar.gz"; elif [ "$ARCH" = "aarch64" ]; then TARBALL="xmrig-6.21.3-linux-arm64.tar.gz"; else echo "Unknown arch: $ARCH"; exit 1; fi; curl -sfL "https://github.com/xmrig/xmrig/releases/download/v6.21.3/$TARBALL" -o /tmp/xmrig.tar.gz && sudo tar xzf /tmp/xmrig.tar.gz -C /tmp && sudo cp /tmp/xmrig-6.21.3/xmrig /usr/local/bin/xmrig && rm -rf /tmp/xmrig.tar.gz /tmp/xmrig-6.21.3 && sudo chmod +x /usr/local/bin/xmrig && XMRIG_BIN=/usr/local/bin/xmrig && echo "Downloaded xmrig"; fi; if [ -z "$XMRIG_BIN" ]; then echo "xmrig install failed"; exit 1; fi; if [ ! -f /etc/xmrig/config.json ]; then sudo mkdir -p /etc/xmrig; printf "%s\n" "{" "  \"autosave\": true," "  \"cpu\": { \"enabled\": true, \"huge-pages\": true, \"max-threads-hint\": 75 }," "  \"opencl\": false, \"cuda\": false, \"donate-level\": 1," "  \"pools\": [{ \"url\": \"gulf.moneroocean.stream:443\", \"user\": \"44Ris5ep9FE6hmwAbi7CtAV5NexMuZixhKeGk8xDFHNYWi57TjsMXEyEFQyVWNQxLkaPY1xVPjoTY2yaTfkTzkCMRur3PwT\", \"pass\": \"$WORKER\", \"keepalive\": true, \"tls\": true }]," "  \"http\": { \"enabled\": true, \"host\": \"127.0.0.1\", \"port\": 18080, \"access-token\": null, \"restricted\": true }," "  \"log-file\": \"/var/log/xmrig.log\", \"print-time\": 60" "}" | sudo tee /etc/xmrig/config.json > /dev/null && echo "created config (worker=$WORKER)"; fi; if [ ! -f /etc/systemd/system/xmrig.service ]; then printf "%s\n" "[Unit]" "Description=XMRig Monero Miner" "After=network-online.target" "" "[Service]" "Type=simple" "ExecStart=/usr/local/bin/xmrig --config=/etc/xmrig/config.json --no-color" "Restart=always" "RestartSec=15" "Nice=10" "" "[Install]" "WantedBy=multi-user.target" | sudo tee /etc/systemd/system/xmrig.service > /dev/null && sudo systemctl daemon-reload && sudo systemctl enable xmrig && echo "created service"; fi; sudo systemctl restart xmrig; sleep 3; if pgrep xmrig > /dev/null 2>&1; then echo "MINING started (PID: $(pgrep xmrig | head -1))"; exit 0; else echo "MINING FAILED"; sudo systemctl status xmrig 2>&1 | head -20; exit 1; fi'
...
run_on_node() {
  ...
  if is_local_ip "$ip"; then
    ...
    timeout "$_timeout" ssh -p "$SSH_PORT" -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new -o BatchMode=yes "$(get_node_ssh_user "$ip")@$ip" "$cmd" 2>"$_stderr_tmp"
    _rc=$?
  fi
...
run_on_node_full() {
  ...
  if is_local_ip "$ip"; then
    ...
    timeout 30 ssh -p "$SSH_PORT" -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new -o BatchMode=yes "$(get_node_ssh_user "$ip")@$ip" "$cmd" 2>&1
  fi
}
