...
function Invoke-Swarm([string]$Action, [string]$Method='GET', $Body=$null, $Config) {
    $headers = @{ Authorization="Bearer $($Config.password)"; 'Content-Type'='application/json' }
    $uri = "$($Config.swarm_url)?action=$([uri]::EscapeDataString($Action))&_=$([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())"
...
function Run-Agent {
    ...
    while ($true) {
        try {
            ...
            $response = Invoke-Swarm -Action 'swarm-poll' -Config $config
