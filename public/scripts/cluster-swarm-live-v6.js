(() => {
  ...
  const PC_IDS = new Set(['Alina','Nexus','SteamDeck']);
  ...
  function syncJobInput() {
    ...
    input.placeholder = needsCommand
      ...
      : type.startsWith('mining-')
        ? 'Phone start/stop uses thermal-safe Windows control; PCs use Swarm'
  ...
  function ensureUi() {
    ...
    if (typeSelect && typeSelect.dataset.v6 !== '1') {
      typeSelect.dataset.v6 = '1';
    ...
    if (dispatchCard && !document.getElementById('swarm-miner-presets')) {
      dispatchCard.insertAdjacentHTML('afterbegin', `
        <div id="swarm-miner-presets" style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end;margin-bottom:10px">
          <button type="button" data-miner-action="status" data-miner-target="__all__">Node Status</button>
          <button type="button" data-miner-action="mining-status" data-miner-target="__all__">Miner Status</button>
          <button type="button" data-miner-action="mining-stop" data-miner-target="__all__">Stop All Miners</button>
          <button type="button" data-miner-action="mining-start" data-miner-target="__phones__">Start Phones (Thermal Safe)</button>
          <button type="button" data-miner-action="mining-stop" data-miner-target="__phones__">Stop Phones</button>
          <button type="button" data-miner-action="mining-start" data-miner-target="__pcs__">Start PCs</button>
        </div>
      `);
      dispatchCard.querySelectorAll('#swarm-miner-presets button').forEach((b) => {
        ...
        b.addEventListener('click', () => runAction(b.dataset.minerAction, b.dataset.minerTarget));
      });
    }
  ...
  function updateTargets(nodes) {
    ...
    select.innerHTML = `
      <option value="__all__">All canonical devices</option>
      <option value="__phones__">All 8 phones</option>
      <option value="__pcs__">Online PCs</option>
      <option disabled>──────────────</option>
    `;
    for (const node of nodes) {
      const option = document.createElement('option');
      option.value = node.id;
      option.disabled = PC_IDS.has(node.id) && !node.online;
  ...
  function render(raw) {
    ...
    if (oldAgents.length) note += ` · Swarm miner controls need v${REQUIRED_AGENT}: ${oldAgents.join(', ')}`;
  ...
  async function load() {
    ...
    requestBusy = true;
    try {
      if (!current) setState('Loading live cluster state…');
  ...
  function normalizeTarget(value) {
    ...
    return ({nexus:'Nexus', steamdeck:'SteamDeck', alina:'Alina'}[lower] || v);
  }
  ...
  function pcTargets(value) {
    const v = normalizeTarget(value);
    const online = (current?.nodes || []).filter((n) => n.online && PC_IDS.has(n.id)).map((n) => n.id);
    if (v === '__all__' || v === 'all' || v === '__pcs__' || v === 'pcs') return online;
    return PC_IDS.has(v) && online.includes(v) ? [v] : [];
  }
  ...
  async function controlPhoneAction(type, targets) {
    ...
    for (const x of mapped) {
      await controlApi('update-desired', 'POST', {
        ...
        desired:{
          ...
          thread_count:x.targetThreads,
        },
      });
    }

    const commandType = type === 'mining-restart' ? 'restart' : type;
    const commandTargets = targets.length === PHONE_IDS.size
      ? ['phones']
      : mapped.map((x) => x.device.id);
    ...
    for (const target of commandTargets) {
      const result = await controlApi('queue-command', 'POST', {
        target,
        type:commandType,
        payload:{ source:'dashboard-v6', thermal_authority:'windows-adb' },
      });
  ...
  async function runAction(type, target = '__all__', cmd = '') {
    try {
      ...
      let pcs = pcTargets(target);

      if ((type === 'mining-start' || type === 'mining-restart') && pcs.includes('Nexus')) {
        pcs = pcs.filter((id) => id !== 'Nexus');
        if (!phones.length && !pcs.length) throw new Error('Nexus mining start is blocked by policy');
      }
  ...
  function patchCommandShortcuts() {
    ...
    if (!box || box.dataset.safeMinerShortcuts === '1') return;
    box.dataset.safeMinerShortcuts = '1';
    box.innerHTML = `
      ...
      <button data-miner-type="mining-start" data-miner-target="__phones__">Start Phones (Thermal Safe)</button>
  ...
  window.submitSwarmJob = async () => {
    ...
    try {
      await runAction(type, target, cmd);
      const input = document.getElementById('swarm-job-cmd');
