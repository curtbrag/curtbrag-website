...
function generateXmrigConfig(desired, device) {
  ...
  const config = {
    ...
    colors: false,
    title: false,
    randomx: { init: -1, mode: desired.randomx_mode || "light", "1gb-pages": false },
    cpu: { enabled: true, "huge-pages": false, "hw-aes": true, priority: 2 },
    "donate-level": 1,
    "donate-over-proxy": 1,
    "log-file": desired.log_path ? desired.log_path.replace(/^~/, process.env.HOME || "~") : `${process.env.HOME || "~"}/cluster/logs/xmrig.log`,
    "print-time": desired.print_interval || 60,
    retries: 5,
    "retry-pause": 5,
    syslog: false,
    verbose: 0,
    ...
    pools: [
      {
        ...
        user: desired.pool_user || "44Ris5ep9FE6hmwAbi7CtAV5NexMuZixhKeGk8xDFHNYWi57TjsMXEyEFQyVWNQxLkaPY1xVPjoTY2yaTfkTzkCMRur3PwT",
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (action === "telemetry" && event.httpMethod === "POST") {
      ...
      const obs = (await openStore("cp-observed").get(deviceId, { type: "json" }).catch(() => null)) || {};
